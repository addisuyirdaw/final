...
import { ArrowRight, Loader, Users, ShieldAlert, Sparkles, X, Upload, Image as ImageIcon } from "lucide-react";
...
export const Union = () => {
  ...
  const fetchUnion = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${API_BASE}/api/staff?pageGroup=student_union`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      data.append("pageGroup", "student_union");
      ...
      if (result.success) {
        toast.success("Union representative profile slot successfully created!");
  ...
  const groupedProfiles = profiles.reduce((groups, profile) => {
    const dept = profile.department || "Executive Committee";
                  ...
                  {deptProfiles.map((profile, index) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200/80 hover:border-blue-200 flex flex-col items-center text-center h-full cursor-pointer transition-all duration-300 ease-in-out hover:scale-[1.03] hover:shadow-[0_10px_30px_rgba(37,99,235,0.08)] group"
                        onClick={() => navigate(`/profile/${profile._id}`)}
                      >
                        {/* Image Container */}
                        <div className="w-40 h-48 rounded-2xl mb-6 overflow-hidden border-4 border-blue-50 bg-gray-50 shadow-md flex items-center justify-center">
                          <img 
                            src={imageUrl} 
                            alt={profile.name} 
            ...
            >
              <div className="sticky top-0 bg-white z-10 px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                <h2 className="text-xl font-bold text-gray-900 flex items-center gap-2">
                  ...
                  Reserve New Union Representative Slot
                </h2>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Academic history, election background, previous roles..."
                    ></textarea>
                  </div>
