...
export function CertificateTemplate({ data = {}, onDispose }) {
  ...
  const certificateNumber = data.certificateNumber || data.certificate_id || "DBU-SS-2026-000001";
  const verification_url = data.verification_url;
  const verifyCode = data.verification_code || "A7QK-91LP-X32M";

  // Always ensure verifyUrl contains /verify/<certificateNumber>
  let verifyUrl = getPublicVerificationUrl(certificateNumber);
  if (verification_url && verification_url.includes("/verify/") && !verification_url.includes("localhost") && !verification_url.includes("127.0.0.1")) {
    verifyUrl = verification_url;
  }

  // Debug logs requested for QR tracing
  console.log("=== CERTIFICATE QR DEBUG LOGS ===");
      ...
      <div className="print-card-shadow w-full max-w-[1080px] aspect-[1.414/1] bg-slate-950 p-[14px] rounded-xl border border-slate-800 shadow-2xl relative overflow-hidden transition-all duration-300 print:rounded-none print:border-none print:shadow-none print:p-0">
        ...
        >
          ...
          <div className="w-full h-full bg-[#fffef9] border-[3px] border-[#c9952a] relative p-6 sm:p-10 flex flex-col justify-between overflow-hidden">
            ...
            <div className="text-center text-[7.5px] font-sans tracking-[0.05em] font-semibold text-gray-500 mt-3 border-t border-gray-200/60 pt-1.5 flex flex-col gap-0.5 px-2">
              <div className="flex items-center justify-between uppercase text-gray-600">
                <span>CERT NO: <strong className="text-gray-800">{certData.certificate_id}</strong> &bull; VERIFY CODE: <strong className="text-gray-800">{certData.verification_code}</strong></span>
                <span>DATE OF ISSUE: {dateString}</span>
                <span className="pr-16">DEBRE BERHAN UNIVERSITY &bull; STUDENT UNION</span>
              </div>
              <p className="text-[6.5px] text-gray-400 font-normal tracking-normal normal-case">
                This certificate was electronically generated by the Debre Berhan University Student Union Management System. Verify authenticity using the QR Code or Certificate Number.
              </p>
            </div>
